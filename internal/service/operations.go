...
type OperationsRepository interface {
	...
	CommitTelemetrySample(context.Context, repository.TelemetryCommit) (bool, error)
	CommitTelemetryIncident(context.Context, repository.TelemetryCommit) error
...
func (s *OperationsService) ingestOne(ctx context.Context, sample telemetry.Sample) telemetry.IngestResult {
	...
	duplicate, err := s.repository.CommitTelemetrySample(ctx, commit)
	if err != nil {
	...
	if duplicate {
		return telemetry.IngestResult{EventID: sample.EventID, Accepted: true, Duplicate: true}
	}
	if commit.Incident != nil {
		if err := s.repository.CommitTelemetryIncident(ctx, commit); err != nil {
			return telemetry.IngestResult{EventID: sample.EventID, Code: "persistence_failed", Message: err.Error()}
		}
	}
