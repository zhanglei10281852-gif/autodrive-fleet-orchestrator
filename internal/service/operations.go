...
type OperationsRepository interface {
	...
	SaveIncidentClaim(context.Context, safety.Incident) error
...
func (s *OperationsService) ClaimIncident(ctx context.Context, principal auth.Principal, id string, expectedVersion int64) (safety.Incident, error) {
	...
	claimed, err := current.Claim(principal.UserID, s.clock.Now(), s.claimLease)
	if err != nil {
		return safety.Incident{}, err
	}
	claimed.UpdatedAt = s.clock.Now()
	if err := s.repository.SaveIncidentClaim(ctx, claimed); err != nil {
		return safety.Incident{}, err
	}
	return claimed, nil
}
