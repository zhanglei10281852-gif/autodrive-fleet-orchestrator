...
type DispatchRepository interface {
	...
	CommitTripAndMissionStart(context.Context, repository.TripStartCommit) error
	CommitVehicleTripStart(context.Context, repository.TripStartCommit) error
...
func (s *DispatchService) StartTrip(ctx context.Context, principal auth.Principal, tripID string) (trip.Trip, error) {
	...
	commit := repository.TripStartCommit{
	...
	if err := s.repository.CommitTripAndMissionStart(ctx, commit); err != nil {
		return trip.Trip{}, err
	}
	if err := s.repository.CommitVehicleTripStart(ctx, commit); err != nil {
		return trip.Trip{}, err
	}
