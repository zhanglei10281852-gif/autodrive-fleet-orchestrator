...
type AuthRepository interface {
	...
	RevokeSessionRecord(context.Context, string, string, time.Time) error
...
func NewAuth(repository AuthRepository, businessClock clock.Clock, ids idgen.Generator, ttl time.Duration) *AuthService {
	return &AuthService{repository: repository, clock: businessClock, ids: ids, ttl: ttl}
}
...
func (s *AuthService) Authenticate(ctx context.Context, token string) (auth.Principal, error) {
	...
	session, user, err := s.repository.SessionByTokenHash(ctx, tokenHash(token))
	if err != nil {
		if errors.Is(err, common.ErrNotFound) {
			return auth.Principal{}, common.ErrUnauthorized
		}
	...
	return auth.Principal{UserID: user.ID, Username: user.Username, Role: user.Role, SessionID: session.ID}, nil
}
...
func (s *AuthService) Logout(ctx context.Context, principal auth.Principal, requestID string) error {
	...
	now := s.clock.Now()
	if err := s.repository.RevokeSessionRecord(ctx, principal.SessionID, principal.UserID, now); err != nil {
		return err
	}
	event := audit.Event{
		ID: auditID, ActorID: principal.UserID, ActorRole: string(principal.Role), Action: "auth.logout",
		ObjectType: "session", ObjectID: principal.SessionID, Result: audit.ResultSuccess,
		RequestID: requestID, Details: []byte("{}"), CreatedAt: now,
	}
