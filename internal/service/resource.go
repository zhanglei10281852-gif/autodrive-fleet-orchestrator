...
type ResourceRepository interface {
	...
	ActivateChargingSession(context.Context, charging.Session) error
	AcquireChargingResources(context.Context, charging.Session, charging.Connector, int64, time.Time, audit.Event) error
...
func (s *ResourceService) StartCharging(ctx context.Context, principal auth.Principal, id string, expectedVersion int64) (charging.Session, error) {
	...
	if err := s.repository.ActivateChargingSession(ctx, started); err != nil {
		return charging.Session{}, err
	}
	if err := s.repository.AcquireChargingResources(ctx, started, connector, vehicle.Version, now, event); err != nil {
		return charging.Session{}, err
	}
	return started, nil
}
