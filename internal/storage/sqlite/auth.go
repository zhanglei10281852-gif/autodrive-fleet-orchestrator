...
func (s *Store) RevokeSessionRecord(ctx context.Context, sessionID, userID string, at time.Time) error {
...
func (s *Store) RevokeSession(ctx context.Context, sessionID, userID, auditID, requestID string, at time.Time) error {
	if err := s.RevokeSessionRecord(ctx, sessionID, userID, at); err != nil {
		return err
	}
	...
	return s.AppendAudit(ctx, audit.Event{
		ID: auditID, ActorID: userID, ActorRole: string(user.Role), Action: "auth.logout",
		ObjectType: "session", ObjectID: sessionID, Result: audit.ResultSuccess,
		RequestID: requestID, Details: []byte("{}"), CreatedAt: at,
	})
}
