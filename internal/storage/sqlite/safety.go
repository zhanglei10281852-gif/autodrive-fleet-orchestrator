...
func (s *Store) SaveIncidentClaim(ctx context.Context, claimed safety.Incident) error {
	result, err := s.db.ExecContext(ctx, `
		UPDATE safety_incidents SET status = ?, owner_id = ?, lease_until = ?, version = ?, updated_at = ?
		WHERE id = ?`,
		claimed.Status, claimed.OwnerID, nullableTime(claimed.LeaseUntil), claimed.Version,
		formatTime(claimed.UpdatedAt), claimed.ID)
	if err != nil {
		return mapSQLError(err, "incident claim")
	}
	return rowsAffectedExactlyOne(result, "incident claim")
}
...
func (s *Store) CommitIncidentResolution(ctx context.Context, commit repository.IncidentResolutionCommit) error {
	return s.WithTx(ctx, func(tx *sql.Tx) error {
		...
		if err := tx.QueryRowContext(ctx, `SELECT status FROM vehicles WHERE id = ?`, commit.Incident.VehicleID).Scan(&vehicleStatus); err != nil {
			return mapSQLError(err, "incident vehicle")
		}
		if fleet.VehicleStatus(vehicleStatus) != commit.VehicleStatus {
			return common.ConflictError{Resource: "vehicle", Reason: "vehicle safety state changed"}
		}
