...
func (s *Store) ActivateChargingSession(ctx context.Context, session charging.Session) error {
	...
	return rowsAffectedExactlyOne(sessionResult, "charging start")
}

func (s *Store) AcquireChargingResources(ctx context.Context, session charging.Session, connector charging.Connector, vehicleVersion int64, at time.Time, event audit.Event) error {
	return s.WithTx(ctx, func(tx *sql.Tx) error {
