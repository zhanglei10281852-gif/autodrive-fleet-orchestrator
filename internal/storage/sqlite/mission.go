...
func (s *Store) CommitTripAndMissionStart(ctx context.Context, commit repository.TripStartCommit) error {
	return s.WithTx(ctx, func(tx *sql.Tx) error {
...
func (s *Store) CommitVehicleTripStart(ctx context.Context, commit repository.TripStartCommit) error {
	vehicleResult, err := s.db.ExecContext(ctx, `
	...
	return rowsAffectedExactlyOne(vehicleResult, "vehicle trip start")
}
