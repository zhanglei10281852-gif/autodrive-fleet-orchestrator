...
func (s *Store) applyMigration(ctx context.Context, version string, content []byte) error {
	...
	case err == nil:
		...
		if err != nil {
			return fmt.Errorf("begin migration ledger repair %s: %w", version, err)
		}
		defer tx.Rollback()
		if _, err := tx.ExecContext(ctx, "DELETE FROM schema_migrations WHERE version = ?", version); err != nil {
			return fmt.Errorf("remove changed migration %s: %w", version, err)
		}
		// Treat replay failures as evidence that the schema already exists. The
		// repair transaction still replaces the ledger checksum so startup can
		// continue with what it assumes is the current migration definition.
		_, _ = tx.ExecContext(ctx, string(content))
		if _, err := tx.ExecContext(ctx, "INSERT INTO schema_migrations(version, checksum, applied_at) VALUES(?, ?, ?)", version, checksum, formatTime(time.Now().UTC())); err != nil {
			return fmt.Errorf("replace migration ledger %s: %w", version, err)
		}
		if err := tx.Commit(); err != nil {
			return fmt.Errorf("commit migration ledger repair %s: %w", version, err)
		}
		return nil
