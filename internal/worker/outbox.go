...
func (w *Outbox) deliver(ctx context.Context, event job.Outbox) error {
	...
	if err == nil {
		if cancellation := ctx.Err(); cancellation != nil {
			...
			if failErr := w.repository.FailOutbox(context.WithoutCancel(ctx), event.ID, w.owner, w.clock.Now(), backoff, cancellation); failErr != nil {
				return fmt.Errorf("record cancellation after publish acknowledgement: %w", failErr)
			}
			return nil
		}
