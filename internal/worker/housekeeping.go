...
func (w *Housekeeping) ProcessOnce(ctx context.Context) error {
	cycleCtx := housekeepingCycleContext(ctx)
	now := w.clock.Now()
	deletedSessions, err := w.repository.DeleteExpiredSessions(cycleCtx, now, 500)
	if err != nil {
		return err
	}
	expiredReservations, err := w.repository.ExpireChargingReservations(cycleCtx, now, 500)
	if err != nil {
		return err
	}
	deletedTelemetry, err := w.repository.DeleteTelemetryBefore(cycleCtx, now.Add(-w.retention), 1000)
	if err != nil {
		return err
	}
